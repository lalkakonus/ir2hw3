...
// CTR, когда документ кликается последним в выдаче по конкретному запросу
public class TLastCTRQueryDoc extends BaseStatistic {
    private static final Text ONE_VIEWED = new Text("VIEWED");
    private static final Text ONE_CLICKED = new Text("CLICKED");

    public TLastCTRQueryDoc(HashMap<String, Integer> URLmap, HashMap<String, Integer> queryMap){
        this.URLMap = URLmap;
        this.QueryMap = queryMap;
        this.KeyPrefix = Config.LAST_CTR_QUERY_DOC_KEY;
    }

    public TLastCTRQueryDoc(){
    ...
    public void map(TDataConverter.TResult Serp, Mapper.Context context, boolean hostFlag) throws IOException, InterruptedException {
        if (!Serp.Clicked.isEmpty()) {
            write(context, hostFlag, Serp.Clicked.get(Serp.Clicked.size() - 1).URL, Serp.Query, ONE_CLICKED);
        }
